...
export async function buildEngine(locale?: SupportedLocale) {
  ...
  const [
    ...
    dl.loadReactions(locale),
    dl.loadActivitySeries(locale).catch(() => []),
    dl.loadClassificationRules(locale).catch(() => []),
    dl.loadNamingRules(locale).catch(() => []),
    dl.loadQualitativeTests(locale).catch(() => []),
    dl.loadEnergyCatalystTheory(locale).catch(() => null),
