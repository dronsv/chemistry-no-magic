...
import { loadEngineAdapter } from './exercise-adapters';
...
export default function CompetencyPracticeIsland({ competencyId, competencyName, locale }: Props) {
  ...
  useEffect(() => {
    Promise.all([
      loadEngineAdapter(competencyId, locale).then(a => {
        if (!a) throw new Error(`No engine adapter for competency: ${competencyId}`);
        return a;
      }),
      loadBktParams(),
    ]).then(([adp, params]) => {
